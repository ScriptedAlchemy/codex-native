...
async function main() {
  ...
  const inputValidationGuardrail = defineOutputGuardrail({
    ...
    validate: async (output: string) => {
      try {
        ...
        if (typeof output === 'string' && output.length >= 10 && output.length <= 1000) {
          return { valid: true };
        }
        return {
          valid: false,
          reason: 'Output must be between 10 and 1000 characters, or a valid JSON object with type, content, and optional priority fields',
        };
      }
    },
    execute: async (output: string) => output,
  });
  ...
  const agentWithValidation = new Agent({
    ...
    outputGuardrails: [inputValidationGuardrail],
  });
  ...
  const contentFilterGuardrail = defineOutputGuardrail({
    ...
    validate: async (output: string) => {
      ...
      const foundTerms = blockedTerms.filter(term => lowerOutput.includes(term));

      if (foundTerms.length > 0) {
        return {
          valid: false,
          reason: `Output contains blocked terms: ${foundTerms.join(', ')}`,
        };
      }
  ...
  const securityGuardrail = defineOutputGuardrail({
    name: 'security-check',
    validate: async (output: string) => {
      for (const pattern of dangerousPatterns) {
        if (pattern.test(output)) {
          return {
            valid: false,
            reason: 'Output contains potentially dangerous commands that could cause data loss',
          };
        }
      }
      return { valid: true };
