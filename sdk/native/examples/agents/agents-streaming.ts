...
async function main() {
  ...
  // Use the model's streaming API directly for better control
  const model = await codexProvider.getModel();
  const request = {
    systemInstructions: streamingAgent.instructions,
    input: 'Explain how streaming works in AI agents',
    modelSettings: {},
    tools: [],
  ...
  const request2 = {
    systemInstructions: progressAgent.instructions,
  ...
  for await (const event of model.getStreamedResponse(request2)) {
    switch (event.type) {
      case 'response_started':
  ...
  const request3 = {
    systemInstructions: customAgent.instructions,
    input: 'List the benefits of using streaming in AI applications',
    modelSettings: {},
    tools: [],
    outputType: { type: 'text' as const },
    handoffs: [],
  ...
  for await (const event of model.getStreamedResponse(request3)) {
    switch (event.type) {
      case 'output_text_delta':
  ...
  const request4 = {
    systemInstructions: errorHandlingAgent.instructions,
    input: 'Explain error handling in streaming',
    modelSettings: {},
    tools: [],
    outputType: { type: 'text' as const },
    handoffs: [],
  ...
  try {
    for await (const event of model.getStreamedResponse(request4)) {
      switch (event.type) {
        case 'output_text_delta':
          process.stdout.write(event.delta);
          break;
    ...
    console.error('\n\n✗ Fatal error during streaming:', error instanceof Error ? error.message : String(error));
  }
  ...
  console.log('  • Use model.getStreamedResponse() for streaming with CodexProvider');
