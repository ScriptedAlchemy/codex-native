/**
 * Test agent handoffs with CodexProvider
 *
 * This test demonstrates multi-agent workflows where one agent can delegate
 * tasks to specialized agents using handoffs.
 */

import { describe, expect, it, beforeAll } from "@jest/globals";
import { fileURLToPath } from "node:url";

function resolveNativeBindingPath() {
  const triplet = (() => {
    const { platform, arch } = process;
    if (platform === "darwin") {
      return arch === "arm64" ? "codex_native.darwin-arm64.node" : "codex_native.darwin-x64.node";
    }
    if (platform === "linux") {
      const suffix = process.env.MUSL ? "musl" : "gnu";
      return `codex_native.${platform}-${arch}-${suffix}.node`;
    }
    if (platform === "win32") {
      return arch === "arm64"
        ? "codex_native.win32-arm64-msvc.node"
        : "codex_native.win32-x64-msvc.node";
    }
    throw new Error(`Unsupported platform for tests: ${platform} ${arch}`);
  })();

  return fileURLToPath(new URL(`../${triplet}`, import.meta.url));
}

process.env.CODEX_NATIVE_BINDING = resolveNativeBindingPath();

let CodexProvider;

beforeAll(async () => {
  ({ CodexProvider } = await import("../dist/index.mjs"));
});

describe("CodexProvider - Agent Handoffs", () => {
  it("works with agent handoffs using mock backend", async () => {
    const { Agent, Runner } = await import("@openai/agents");
    const {
      startResponsesTestProxy,
      sse,
      responseStarted,
      assistantMessage,
      responseCompleted,
    } = await import("./responsesProxy.mjs");

    // Set up mock responses for the handoff flow
    // Response 1: Primary agent recognizes need for specialist and hands off
    // Response 2: Specialist agent handles the task
    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        // First response: Primary agent delegates to code specialist
        sse(
          responseStarted("response_1"),
          assistantMessage(
            "I see you need code review. Let me hand this off to the Code Specialist.",
            "item_1"
          ),
          responseCompleted("response_1")
        ),
        // Second response: Code specialist handles the review
        sse(
          responseStarted("response_2"),
          assistantMessage(
            "I've reviewed the code and found: The function looks good, but could use better error handling.",
            "item_2"
          ),
          responseCompleted("response_2")
        ),
      ],
    });

    try {
      const provider = new CodexProvider({
        baseUrl: url,
        apiKey: "test",
        skipGitRepoCheck: true,
      });

      // Create specialized code review agent
      const codeSpecialist = new Agent({
        name: "CodeSpecialist",
        instructions: "You are an expert code reviewer. You analyze code quality, security, and best practices.",
        handoffDescription: "Expert at code review, refactoring, and identifying bugs",
      });

      // Create primary agent with handoff to specialist
      const primaryAgent = new Agent({
        name: "PrimaryAgent",
        instructions: "You are a general-purpose assistant. Delegate code review tasks to the CodeSpecialist.",
        handoffs: [codeSpecialist],
      });

      const runner = new Runner({ modelProvider: provider });
      const result = await runner.run(primaryAgent, "Can you review this function for me?");

      // Verify the runner got a result
      expect(result).toBeDefined();
      expect(result.finalOutput).toBeDefined();

      // Verify handoff occurred by checking multiple requests were made
      expect(requests.length).toBeGreaterThanOrEqual(1);

      // The result should contain specialist's analysis
      expect(result.finalOutput.toLowerCase()).toMatch(/code|review|function/i);
    } finally {
      await close();
    }
  }, 20000); // Longer timeout for multi-agent workflow

  it("works with multiple specialized agents", async () => {
    const { Agent, Runner } = await import("@openai/agents");
    const {
      startResponsesTestProxy,
      sse,
      responseStarted,
      assistantMessage,
      responseCompleted,
    } = await import("./responsesProxy.mjs");

    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        // Response from bug fixer
        sse(
          responseStarted("response_1"),
          assistantMessage("Bug fixed! The issue was a null pointer exception.", "item_1"),
          responseCompleted("response_1")
        ),
        // Response from test writer
        sse(
          responseStarted("response_2"),
          assistantMessage("Tests written! Added unit tests for edge cases.", "item_2"),
          responseCompleted("response_2")
        ),
      ],
    });

    try {
      const provider = new CodexProvider({
        baseUrl: url,
        apiKey: "test",
        skipGitRepoCheck: true,
      });

      // Create multiple specialized agents
      const bugFixer = new Agent({
        name: "BugFixer",
        instructions: "You fix bugs in code",
        handoffDescription: "Specialist in debugging and fixing code issues",
      });

      const testWriter = new Agent({
        name: "TestWriter",
        instructions: "You write comprehensive tests",
        handoffDescription: "Expert at writing unit and integration tests",
      });

      // Primary agent can delegate to multiple specialists
      const coordinator = new Agent({
        name: "Coordinator",
        instructions: "You coordinate work between specialists",
        handoffs: [bugFixer, testWriter],
      });

      const runner = new Runner({ modelProvider: provider });
      const result = await runner.run(coordinator, "Fix bugs and add tests");

      expect(result).toBeDefined();
      expect(result.finalOutput).toBeDefined();
    } finally {
      await close();
    }
  }, 20000);

  it("handles handoff chain (agent A -> agent B -> agent C)", async () => {
    const { Agent, Runner } = await import("@openai/agents");
    const {
      startResponsesTestProxy,
      sse,
      responseStarted,
      assistantMessage,
      responseCompleted,
    } = await import("./responsesProxy.mjs");

    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
          responseStarted("response_1"),
          assistantMessage("Coordinator: Delegating to Analyzer...", "item_1"),
          responseCompleted("response_1")
        ),
        sse(
          responseStarted("response_2"),
          assistantMessage("Analyzer: Analysis complete. Delegating to Fixer...", "item_2"),
          responseCompleted("response_2")
        ),
        sse(
          responseStarted("response_3"),
          assistantMessage("Fixer: All issues resolved!", "item_3"),
          responseCompleted("response_3")
        ),
      ],
    });

    try {
      const provider = new CodexProvider({
        baseUrl: url,
        apiKey: "test",
        skipGitRepoCheck: true,
      });

      // Create a chain of agents: Coordinator -> Analyzer -> Fixer
      const fixer = new Agent({
        name: "Fixer",
        instructions: "You fix identified issues",
        handoffDescription: "Fixes problems found during analysis",
      });

      const analyzer = new Agent({
        name: "Analyzer",
        instructions: "You analyze code for issues",
        handoffDescription: "Analyzes code and identifies problems",
        handoffs: [fixer],
      });

      const coordinator = new Agent({
        name: "Coordinator",
        instructions: "You coordinate the workflow",
        handoffs: [analyzer],
      });

      const runner = new Runner({ modelProvider: provider });
      const result = await runner.run(coordinator, "Analyze and fix this code");

      expect(result).toBeDefined();
      expect(result.finalOutput).toBeDefined();
      // Verify the workflow ran (could be coordinator, analyzer, or fixer)
      expect(result.finalOutput.toLowerCase()).toMatch(/coordinat|analyz|fix|resolve|complet/i);
    } finally {
      await close();
    }
  }, 25000);

  it("verifies handoff data is passed in ModelRequest", async () => {
    const { Agent, Runner } = await import("@openai/agents");
    const {
      startResponsesTestProxy,
      sse,
      responseStarted,
      assistantMessage,
      responseCompleted,
    } = await import("./responsesProxy.mjs");

    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
          responseStarted("response_1"),
          assistantMessage("Task completed!", "item_1"),
          responseCompleted("response_1")
        ),
      ],
    });

    try {
      const provider = new CodexProvider({
        baseUrl: url,
        apiKey: "test",
        skipGitRepoCheck: true,
      });

      const specialist = new Agent({
        name: "Specialist",
        instructions: "You are a specialist",
        handoffDescription: "Handles specialized tasks",
      });

      const primary = new Agent({
        name: "Primary",
        instructions: "You are the primary agent",
        handoffs: [specialist],
      });

      const runner = new Runner({ modelProvider: provider });
      await runner.run(primary, "Do something");

      // Verify that our provider received the handoff information
      // (The framework serializes handoffs and passes them in ModelRequest)
      expect(requests.length).toBeGreaterThan(0);

      // The request should have been made through our provider
      const firstRequest = requests[0];
      expect(firstRequest).toBeDefined();
    } finally {
      await close();
    }
  }, 15000);
});
