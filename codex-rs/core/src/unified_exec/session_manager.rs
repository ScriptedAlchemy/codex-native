...
impl UnifiedExecSessionManager {
    ...
    > {
        ...
        let write_guard = PendingWriteGuard::new(
            Arc::clone(&entry.pending_writes),
            Arc::clone(&entry.write_notify),
        );
        let writer_tx = entry.session.writer_sender();
        let session = Arc::clone(&entry.session_ref);
        let turn = Arc::clone(&entry.turn_ref);
